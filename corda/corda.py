...
def __init__(self, model, confidence, met_prod=None, flux_target=1, n=10, 
    ...
    conf = dict(zip((r.id for r in self.model.reactions), confidence))
    ...
    for r in self.model.reactions:
        ...
        if r.id in conf: self.conf[r.id] = conf[r.id]
        else: 
            rev = self.model.reactions.get_by_id(r.notes["reflection"])
            self.conf[r.id] = conf[rev]
    ...
    self.m_pen_id = next(i for i, r in enumerate(m.reactions) \
        if r.name == "penalty")
...
def __perturb(self, lp, m):
    noise = np.random.uniform(self.noise, size=len(m.reactions))
    
    for i, r in enumerate(m.reactions):
        if self.m_pen in r.metabolites:
            self.corda_solver.change_coefficient(lp, self.m_pen_id, 
                i, r.metabolites[self.m_pen] + noise[i])

def associated(self, targets, conf=self.conf):
    """Gets the associated reactions for the target reactions"""
    
    tidx = ((i, r.id) for i, r in enumerate(model.reactions) if r.id in targets)
    m = model.copy()
    m.add_reactions(self.r_pen)
    
    for r in m.reactions:
        if conf[r.id] == 2 or conf[r.id] == 1: r.add_metabolites({self.m_pen: np.sqrt(penalty)})
        elif conf[r.id] == -1: r.add_metabolites({self.m_pen: penalty})
    
    lp = self.solver.create_problem(self.model)
    lp_corda = self.corda_solver.create_problem(m)
    
    needed = {}
    for ti, rid in tidx:
        solver.change_variable_objective(lp, ti, 1.0)
        sol = solver.solve_problem(lp, objective_sense="maximize", **self.sargs)
        sol = solver.format_solution(lp, self.model)
        if(sol.f < TOL): 
            raise ValueError("Reaction {} can never carry flux!".format(rid))
        solver.change_variable_objective(lp, ti, 0.0)
        
        upper = m.reactions.get_by_id(rid).upper_bound
        corda_solver.change_variable_bounds(corda_lp, ti, self.tflux, upper)
        needed[rid] = np.array()
        for i in range(self.n):
            sol = corda_solver.solve_problem(corda_lp, 
                objective_sense="minimize", **self.sargs)
            sol = corda_solver.format_solution(corda_lp, m)
            need = [r for r in sol.x_dict if sol.x_dict[r] > TOL]
            need = np.hstack([needed[rid], need])
            needed[rid] = np.unique(need)
        
        corda_solver.change_variable_bounds(corda_lp, ti, 0.0, upper)
...
def build(self):
    ...
    include = [r.id for r in self.model.reactions if conf[r.id] == 3]
    need = associated(include)
    ...
    include = [r.id for r in self.model.reactions if conf[r.id] == 1 \
        or conf[r.id] == 2
    need = associated(include)
    ...
    lp = self.solver.create_problem(self.model)
    for i, r in self.model.reactions:
    ...
    for rid, co in self.conf.items():
        ...
        if co == 0:
            self.conf[rid] = -1
    need = associated([k for k in self.conf if self.conf[k] == 3])
    add = np.unique([x for v in need.values() for x in v])
    for a in add: self.conf[a] = 3
